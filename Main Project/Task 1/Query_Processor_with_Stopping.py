...
import time
import Query_Parser_with_Stopping as qp
import collections
import operator
import math
...
bm25_res_dir="./bm25_result_with_stopping/"
rel_doc="../cacm.rel"
#query_dict={}
...
def find_query(task_name):
    ...
    for query_id in query_file_dict:
        ...
        RI=create_RI_dict(query_id,query)
        if query_id in R:
...
def process_query(query,ri,rel_dc_cnt):
    ...
    for doc in BM25_With_Stopping.tf_dict:
        bm25_doc_sum = 0
        for term in query_dict:
            p1=0
            p2=0
            p3=0
            if term in BM25_With_Stopping.tf_dict[doc]:
               # print "ri for " + str(term) + " is " + str(ri[term])
                ...
                p1 = float((ri[term] +0.5)*(BM25_With_Stopping.N - BM25_With_Stopping.doc_table[term] - rel_dc_cnt + ri[term]+ 0.5)) / \
                     ((rel_dc_cnt - ri[term] + 0.5)*(BM25_With_Stopping.doc_table[term] - ri[term] + 0.5))
                p2 = float((BM25_With_Stopping.k1 + 1) * BM25_With_Stopping.tf_dict[doc][term]) / (BM25_With_Stopping.k[doc] + BM25_With_Stopping.tf_dict[doc][term])
                p3 = float((BM25_With_Stopping.k2 + 1) * query_dict[term]) / (BM25_With_Stopping.k2 + query_dict[term])
    ...
    for chunk in chunks:
        ...
        if queryid in R:
            for page in R[queryid]:
                file=BM25_With_Stopping.parsed_corpus_dir+page+".txt"
